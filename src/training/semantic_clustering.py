...
# Chargement
df = pd.read_csv("results/embeddings/node2vec.csv")
...
df['tsne_1'] = X_tsne[:, 0]
df['tsne_2'] = X_tsne[:, 1]
df.to_csv("results/embeddings/node2vec_clustered.csv", index=False)
...
plt.tight_layout()
plt.savefig("results/embeddings/clustering_semantic.png", dpi=150)
plt.show()
